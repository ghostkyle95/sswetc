...
if (instance_exists(baddieID) && place_meeting(x, y, obj_player) && !obj_player.cutscene && obj_player.state != states.hurt)
{
	if (baddieID.state != baddiestates.grabbed)
	{
		with (obj_player)
		{
			...
			if (instance_exists(other.baddieID) && (state == states.mach2 || state == states.rocketfistpizzano || state == states.pizzanoshoulderbash || state == states.jetpackdoise || sprite_index == spr_playerN_divebomb || sprite_index == spr_playerN_divebombland || sprite_index == spr_playerN_divebombfall || sprite_index == spr_playerN_wallbounce ) && other.baddieID.grounded)
			{
				if (other.baddieID.object_index == obj_pizzaball)
					global.golfhit += 1;
				global.hit += 1;
